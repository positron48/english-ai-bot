...
func NewTTSStatusRepository(db *sql.DB, logger *zap.Logger, maxAttempts int) *TTSStatusRepository {
	return NewTTSStatusRepositoryForCourse(db, logger, maxAttempts, "")
}

func NewTTSStatusRepositoryForCourse(db *sql.DB, logger *zap.Logger, maxAttempts int, courseCode string) *TTSStatusRepository {
	if maxAttempts <= 0 {
		maxAttempts = 3
	}
	return &TTSStatusRepository{db: db, logger: logger, maxAttempts: maxAttempts, courseCode: strings.ToLower(strings.TrimSpace(courseCode))}
}
...
func (r *TTSStatusRepository) GetByWord(word string) (*models.TTSGenerationStatus, error) {
	...
	const q = `SELECT course_code, word, state, attempt_count, max_attempts, last_error_code, last_error_message,
		last_provider, audio_rel_path, last_attempt_at, created_at, updated_at
		FROM tts_generation_status
		WHERE course_code = ? AND word = ?`

	var s models.TTSGenerationStatus
	var lastErrorCode, lastErrorMessage, lastProvider, audioRelPath sql.NullString
	var lastAttemptAt sql.NullTime
	err := r.db.QueryRow(q, r.courseCode, normalized).Scan(
		&s.CourseCode, &s.Word, &s.State, &s.AttemptCount, &s.MaxAttempts,
...
func (r *TTSStatusRepository) UpsertPending(word string) error {
	...
	const q = `INSERT INTO tts_generation_status (course_code, word, state, attempt_count, max_attempts, created_at, updated_at)
		VALUES (?, ?, ?, 0, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
		ON CONFLICT(course_code, word) DO UPDATE SET
			state = excluded.state,
			updated_at = CURRENT_TIMESTAMP`
	if _, err := r.db.Exec(q, r.courseCode, normalized, models.TTSStatePending, r.maxAttempts); err != nil {
...
func (r *TTSStatusRepository) MarkAttempt(word, provider, errorCode, errorMessage string, retryable bool) error {
	...
	const upsert = `INSERT INTO tts_generation_status (
			course_code, word, state, attempt_count, max_attempts, last_error_code, last_error_message, last_provider, last_attempt_at, created_at, updated_at
		...
		ON CONFLICT(course_code, word) DO UPDATE SET
			...
			updated_at = CURRENT_TIMESTAMP`
	if _, err := r.db.Exec(upsert, r.courseCode, normalized, state, r.maxAttempts, nullableString(errorCode), nullableString(errorMessage), nullableString(provider)); err != nil {
		return fmt.Errorf("mark tts attempt: %w", err)
	}

	const capToTerminal = `UPDATE tts_generation_status
		SET state = ?, updated_at = CURRENT_TIMESTAMP
		WHERE course_code = ? AND word = ? AND attempt_count >= max_attempts`
	if _, err := r.db.Exec(capToTerminal, models.TTSStateFailedTerminal, r.courseCode, normalized); err != nil {
...
func (r *TTSStatusRepository) MarkReady(word, provider, relPath string) error {
	...
	const q = `INSERT INTO tts_generation_status (
			course_code, word, state, attempt_count, max_attempts, last_provider, audio_rel_path, last_error_code, last_error_message, created_at, updated_at
		...
		ON CONFLICT(course_code, word) DO UPDATE SET
			...
			updated_at = CURRENT_TIMESTAMP`
	if _, err := r.db.Exec(q, r.courseCode, normalized, models.TTSStateReady, r.maxAttempts, nullableString(provider), nullableString(relPath)); err != nil {
...
func (r *TTSStatusRepository) MarkTerminal(word, provider, errorCode, errorMessage string) error {
	...
	const q = `INSERT INTO tts_generation_status (
			course_code, word, state, attempt_count, max_attempts, last_provider, last_error_code, last_error_message, last_attempt_at, created_at, updated_at
		...
		ON CONFLICT(course_code, word) DO UPDATE SET
			...
			last_attempt_at = CURRENT_TIMESTAMP,
			updated_at = CURRENT_TIMESTAMP`
	if _, err := r.db.Exec(q, r.courseCode, normalized, models.TTSStateFailedTerminal, r.maxAttempts, nullableString(provider), nullableString(errorCode), nullableString(errorMessage)); err != nil {
...
func (r *TTSStatusRepository) ResetForForceRegenerate(word string) error {
	...
	const q = `INSERT INTO tts_generation_status (
			course_code, word, state, attempt_count, max_attempts, last_error_code, last_error_message, last_provider, audio_rel_path, created_at, updated_at
