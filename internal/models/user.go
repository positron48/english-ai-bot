...
type UserSettings struct {
	...
	// EnabledVerbScopes controls which verb scope(s) are allowed in verb-forms training (union).
	// The web UI saves a cumulative ladder (see SpanishVerbScopeLadder), e.g. presente+pretérito.
	// Keys match Jehle DB: "es.<tense>.<mood>" such as "es.presente.indicativo", "es.pretérito.indicativo".
	EnabledVerbScopes []string `json:"enabled_verb_scopes,omitempty"`
}
