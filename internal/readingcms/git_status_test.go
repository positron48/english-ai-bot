...
func TestListPublishedMarksUncommittedCoverAsset(t *testing.T) {
	if _, err := exec.LookPath("git"); err != nil {
		t.Skip("git not available")
	}
	f := setupServerFixture(t)
	...
	textID := "pub_es_a2_cover_asset"
	doc := &TextDocument{
		ID:             textID,
		Title:          "Plaza con imagen",
		Level:          "A2",
		TargetLanguage: "es",
		CategoryID:     "es_a2",
		ReadingPassage: map[string]interface{}{
	...
	if out, err := exec.Command("git", "-C", f.root, "add", ".").CombinedOutput(); err != nil {
		t.Fatalf("git add: %v output=%s", err, string(out))
	}
	commit := exec.Command("git", "-C", f.root,
		"-c", "user.email=readingcms@example.test",
		"-c", "user.name=Reading CMS",
		"commit", "-m", "seed")
	if out, err := commit.CombinedOutput(); err != nil {
		t.Fatalf("git commit: %v output=%s", err, string(out))
	}

	assetDir := filepath.Join(f.root, "courses", "spanish-grammar", "assets", "reading", textID)
	if err := os.MkdirAll(assetDir, 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(assetDir, "cover_thumb.webp"), []byte("mock"), 0o644); err != nil {
		t.Fatal(err)
	}
