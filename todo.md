...
7. [done] Проверка правильности ответов в тесте на определение кривая какая-то: для true_false при ответе «Нет» и правильном «Да» показывало Incorrect с Correct Answer: Нет. Исправлено: нормализация Да/Нет, true/false, bool при проверке и в correct_answer в результатах; formatAnswer поддерживает boolean.

7a. [done] В placement-тесте correct_answer и explanation попадали от другого вопроса (напр. «or» помечалось incorrect с Correct: «went»). Причина: один и тот же id (q26) в разных главах — в questionMap перезаписывался. Сделан составной id: chapterID:qid в GeneratePlacementTest и при построении questionMap в SubmitPlacementTest.

8. [done] Для вопросов где нужно вписывать строку нужно добавить кнопку подтверждения, как в квизах. Если такой вопрос последний - по нажатию/подтверждению ввода сразу отправлять тест и показывать результаты

9. [done] в результатах теста на определение уровня нужно выводить шкалу из вопросов горизонтальную. Кружок на каждый вопрос - зеленый или красный в зависимости от правильности ответа, внутри кружка - уровень, к которому привязан вопрос (a0, a1 и тп). Сортировка по порядку следования глав, к которым привязаны вопросы. По клику на вопрос - скролл к конкртеному вопросу с ответами. Отображение результатов тоже должно идти в том же порядке.

10. [done] У нас сейчас в тесте на определение уровня рандомные 20-30 вопросов, давай сделаем стабильное число — 25.

11. [done] при смене вопроса в тесте - и тесте на главу и тесте на определение уровня - если в вопросе нужно что-то ввести в инпут - надо ставить фокус в инпут.
Кнопку check в тестах нужно переименовать, в квизах внутри главы check - ок