...
- Phase 9/unified DB merge foundation: migration `000020_linglow_legacy_merge_mappings.sql`, `cmd/merge_language_databases` low-memory dry-run audit; prod audit/merge через isolated Job 384–512Mi (не `kubectl exec` в app pod). Write phases: `users`, `user-courses`, `course-mappings`, `content`, `attempts`, `srs`, `tts`.
- TTS в unified DB изолирован migration `000024_course_scoped_tts.sql`: ключ статуса `(course_code, word)`, файлы лежат под `<course_code>/...`; `--phase=tts` переносит статусы и переписывает `audio_rel_path`, а старые English/Spanish PVC копируются в `en_ru/` и `es_ru/` по release runbook.
- Phase 9/content merge дополнен переносом legacy Word Sets: `word_set_categories`, `word_sets`, `word_set_items` копируются в unified DB с `course_code` и remap `word_card_id`. Для staging snapshot от 2026-06-09 после выката исправленного образа достаточно повторить идемпотентную команду `merge_language_databases --commit --phase=content`; остальные merge-фазы повторять не требуется.
- Phase 9/content merge repair: source `word_cards` теперь сразу получают `en_ru`/`es_ru`; повторный content merge исправляет ошибочный fallback tag у слов, которые существуют только в одной source DB. Это устраняет случай, когда Spanish legacy word без canonical mapping (например `prepararse`) startup-backfill помечал как `en_ru`.
...
5. Ввести flags:
   ...
   - `GET /api/linglow/review` и `GET /api/linglow/daily-route` используют gated read source и отдают `summary.read_source` - готово.
