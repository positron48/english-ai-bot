...
- Phase 4/grammar dual-write foundation: добавлен feature flag `LINGLOW_EVENTS_WRITE_ENABLED`; при включении online/offline grammar test attempts и Grammar Training SRS attempts зеркалятся в `exercise_attempts` и `learning_events` non-blocking, старые таблицы остаются source of truth.
...
1. Написать команды:
   - `cmd/backfill_linglow_word_progress`
   - `cmd/backfill_linglow_grammar_progress`
...
2. Каждая команда:
   ...
   - `--course-code`;
   - `--since` optional;
   - идемпотентность через deterministic backfill event ids;
