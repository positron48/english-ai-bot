...
- Phase 4/grammar test dual-write foundation: добавлен feature flag `LINGLOW_EVENTS_WRITE_ENABLED`; при включении online/offline grammar test attempts зеркалятся в `exercise_attempts` и `learning_events` non-blocking, старые таблицы остаются source of truth.
