...
- правило: “не ставить рядом карточки с одинаковым `word_card_id`”
...
- `/reset_circuit` — сбросить circuit breaker
  ...
  - При вызове не-админом: игнорировать или ответить "Unknown command"
...
Служебные команды (`/get_id`, `/reset_circuit`) обрабатываются в коде, но не регистрируются.
