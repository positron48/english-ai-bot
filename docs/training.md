...
1. **Слова из текущей сессии тренировки** (1-2 слова) — правильные ответы из других карточек в этой же сессии
   ...
   - Для EN→RU: русские переводы (`MeaningRU`) из других карточек
...
- Дистракторы должны быть схожи по длине и стилю с правильным ответом
  - Если `meaning_ru` — одно слово, дистракторы тоже должны быть однословными
...
**Требования к переводу (`meaning_ru`):**
- Должен быть кратким: максимум 1-2 слова, только основной перевод
- НЕ включать несколько синонимов через запятую (например, использовать "бежать", а не "бежать, бегать, мчаться")
- Выбирать наиболее распространенный однословный перевод
...
1. **Краткость перевода (`meaning_ru`):**
   - Максимум 1-2 слова, только основной перевод
   - НЕ включать несколько синонимов через запятую
   - Пример: использовать "бежать", а не "бежать, бегать, мчаться"
