#!/usr/bin/env python3
"""Read-only validation of the EN 103-106 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-en103106-baseline')
...
units = [
    ...
    if u['language'] == 'en'
    and u['order'].startswith(('103.', '104.', '105.', '106.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 24 and sum(u['count'] for u in units) == 431
...
assert totals == Counter({'unchanged': 314, 'content_changed': 117}), totals
print('Changes:', dict(totals))
print('PASS: all 431 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
