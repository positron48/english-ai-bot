#!/usr/bin/env python3
"""Read-only validation of the EN 079-082 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-en7982-baseline')
...
units = [
    ...
    if u['language'] == 'en'
    and u['order'].startswith(('079.', '080.', '081.', '082.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 28 and sum(u['count'] for u in units) == 495
...
print('PASS: all 495 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
