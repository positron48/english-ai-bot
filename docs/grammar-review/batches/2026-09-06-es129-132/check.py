#!/usr/bin/env python3
"""Read-only validation of the ES 129-132 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es129132-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('129.', '130.', '131.', '132.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 4 and sum(u['count'] for u in units) == 253
...
for i, unit in enumerate(units):
    ...
    if unit['order'].startswith('129.'):
        assert by_id['q129_b1_mcq_e']['prompt'].endswith('___.*')
        assert 'futuro simple' in by_id['q129_b1_mcq_h']['choices'][3]['text']
        assert by_id['q129_b1_err']['choices'][2]['text'] == '*Por más que estudiarás, no aprobarás.*'
        assert by_id['q129_b4_transform']['correct_answer'] == 'Por más que estudies, no aprobarás.'
    if unit['order'].startswith('130.'):
        assert by_id['q130_b1_fill']['correct_answer'] == 'está'
        assert by_id['q130_b4_fill']['correct_answer'] == 'estén'
        assert 'гайя' not in by_id['q130_b3_fill']['explanation']
    if unit['order'].startswith('131.'):
        assert by_id['q131_b1_transform']['correct_answer'] == 'salir'
        assert 'мог бы переводить' in by_id['q131_b5_transform']['prompt']
    if unit['order'].startswith('132.'):
        assert by_id['q132_b1_mcq_e']['choices'][0]['text'] == '*para que* + личная форма в сослагательном'
        assert by_id['q132_b4_cloze']['correct_answer'] == 'envíes|apruebe'
        assert by_id['q132_b2_err']['prompt'].startswith('Говорящий закрывает дверь')
        assert by_id['q132_b3_err']['prompt'].startswith('Исправьте только глагольную форму')
        assert by_id['q132_b4_err']['prompt'].startswith('Исправьте только глагольную форму')
        assert by_id['q132_b3_reorder']['prompt'].startswith('Дополните указание: «Asegúrate')
        assert by_id['q132_b2_transform']['correct_answer'] == 'Simplifico el protocolo para que nadie tenga que repetir el trámite.'
        assert by_id['q132_b3_explain']['prompt'].startswith('*Asegúrate de que la válvula')
...
assert totals == Counter({'content_changed': 23, 'unchanged': 230}), totals
print('Changes:', dict(totals))
print('PASS: all 253 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
