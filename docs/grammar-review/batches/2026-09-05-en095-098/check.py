#!/usr/bin/env python3
"""Read-only validation of the EN 095-098 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-en9598-baseline')
...
units = [
    ...
    if u['language'] == 'en'
    and u['order'].startswith(('095.', '096.', '097.', '098.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 27 and sum(u['count'] for u in units) == 488
...
assert totals == Counter({'unchanged': 259, 'content_changed': 229}), totals
