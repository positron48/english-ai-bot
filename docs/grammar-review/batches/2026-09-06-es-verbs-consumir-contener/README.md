# ES verb forms: consumir, contar, contener

Дата редакторской проверки: 2026-09-06. Независимая проверка ожидается.

## Объём

- `consumir`: 96/96 карточек проверено, `fixed` 96.
- `contar`: 96/96 карточек проверено, `fixed` 96.
- `contener`: 96/96 карточек проверено, `fixed` 96.
...
верхнеуровневые поля сохранены. Для `contar` закреплено значение подсчёта, для
`contener` — сдерживание угроз. Источники точечно синхронизированы с
`internal/grammartrainingpack`; `make training-pack` не запускался.

## Основные исправления

- Все формы сверены с локальной базой Jehle. Исправлена 31 неверная
  `surface_form`: 6 в `consumir`, 18 в `contar` и 7 в `contener`.
- В `consumir` восстановлены все шесть форм futuro de subjuntivo.
- В `contar` восстановлены pretérito anterior, futuro de subjuntivo и futuro
  perfecto de subjuntivo.
- В `contener` исправлены `contenéis` и весь futuro de subjuntivo с основой
  `tuv-`.
- В исходных файлах обнаружено 33 карточки с повторяющимися вариантами. Теперь
...
- Исправлены шаблонные контексты и переводы без нужного временного значения.
  Для отрицательного условия `no contuviere` перевод также содержит отрицание.
  Редкие времена отмечены как книжные, процедурные, тарифные или регламентные
  употребления.
...
- `python3 docs/grammar-review/batches/2026-09-06-es-verbs-consumir-contener/check.py`
...
- Сохранность 25002 файлов вне разрешённого набора — 0 изменённых, 0 пропавших.
