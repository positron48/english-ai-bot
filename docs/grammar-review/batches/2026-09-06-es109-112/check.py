#!/usr/bin/env python3
"""Read-only validation of the ES 109-112 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es109112-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('109.', '110.', '111.', '112.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 4 and sum(u['count'] for u in units) == 264
...
for i, unit in enumerate(units):
    ...
    if unit['order'].startswith('112.'):
        by_id = {question['id']: question for question in questions}
        assert by_id['q064']['correct_answer'] == 'false', 'ES 112 q064 truth-key regression'
        assert 'el equipo' in by_id['q028']['prompt'], 'ES 112 q028 subject regression'
        assert 'для *tú*' in by_id['q041']['prompt'], 'ES 112 q041 imperative-person regression'
...
assert totals == Counter({'content_changed': 25, 'unchanged': 239}), totals
print('Changes:', dict(totals))
print('PASS: all 264 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
