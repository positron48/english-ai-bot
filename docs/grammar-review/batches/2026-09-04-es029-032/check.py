#!/usr/bin/env python3
"""Read-only validation of the ES chapters 029–032 editorial batch against its baseline."""
import json
import pathlib
...
units = [u for u in gr.inventory() if u["language"] == "es" and u["order"].startswith(("029.", "030.", "031.", "032.")) and u["bank"] != "verbs"]
assert len(units) == 28 and sum(u["count"] for u in units) == 488
BASELINES = {
...
def baseline(path):
    if path == ROOT / "courses/spanish-grammar/chapters/029.es.grammar.present_regular_actions.present_statements_regular_verbs/03-questions.json":
        return json.loads((pathlib.Path(__file__).parent / "029-source-before.json").read_text())
...
print('PASS: all 488 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
