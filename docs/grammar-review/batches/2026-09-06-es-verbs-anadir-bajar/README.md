# ES verb forms: añadir, bailar, bajar

Дата редакторской проверки: 2026-09-06. Независимая проверка ожидается.

## Объём

- `añadir`: 96/96 карточек проверено, `fixed` 96.
- `bailar`: 96/96 карточек проверено, `fixed` 96.
- `bajar`: 96/96 карточек проверено, `fixed` 96.
...
- Все формы сверены с локальной базой Jehle. Исправлено 40 неверных
  `surface_form`: 19 в `añadir`, 8 в `bailar` и 13 в `bajar`.
- В `añadir` восстановлены все формы pretérito anterior и futuro perfecto de
  subjuntivo, а также ошибочные формы imperfecto и futuro de subjuntivo.
- В `bailar` восстановлены диакритика в `bailéis`, форма `bailáremos` и весь
  futuro perfecto de subjuntivo.
- В `bajar` исправлены `bajarais`, весь futuro de subjuntivo и весь futuro
  perfecto de subjuntivo.
- В исходных файлах обнаружено 44 карточки с повторяющимися вариантами. Теперь
...
- `python3 docs/grammar-review/batches/2026-09-06-es-verbs-anadir-bajar/check.py`
...
- Сохранность 24890 файлов вне разрешённого набора — 0 изменённых, 0 пропавших.
