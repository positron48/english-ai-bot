#!/usr/bin/env python3
"""Evidence checks for the suceder/sufrir/superar/suponer review batch."""
...
BASE = Path("/tmp/grammar-es-verbs-suceder-suponer-baseline")
LEMMAS = {
    "suceder": "09c75bd66b82c950d3902d1b",
    "sufrir": "aa9a82ac3d6a62dd54ba3880",
    "superar": "3d3ccbe3f2284c4f0c2c3afd",
    "suponer": "8724239c01a3d1a7a71500fc",
}
SCOPES = [
...
def main():
    ...
    for lemma, report_id in LEMMAS.items():
        ...
        for scope, mood, tense in SCOPES:
            ...
            assert [card["surface_form"] for card in cards] == expected, f"{lemma} {scope}: differs from RAE"
        ...
        print(f"{lemma}: PASS 96 cards, exact RAE forms, contracts and report fingerprint")
    checkpoints = [
        json.loads(line)
        for line in (ROOT / "docs/grammar-review/batches/2026-09-06-es-verbs-suceder-suponer/checkpoints.jsonl").read_text().splitlines()
    ]
