#!/usr/bin/env python3
"""Read-only validation of the ES 117-120 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es117120-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('117.', '118.', '119.', '120.'))
    and u['bank'] != 'verbs'
]
...
for i, unit in enumerate(units):
    ...
    if unit['order'].startswith('117.'):
        assert 'En 2010 se mudó' in by_id['q51']['choices'][0]['text']
        assert 'ha querido' in by_id['q29']['choices'][2]['text']
    if unit['order'].startswith('118.'):
        assert by_id['q7']['choices'][3]['text'].endswith('*salg-*')
        assert '*traer*' in by_id['q48']['prompt']
        assert by_id['q60']['correct_answer'] == 'false'
        assert 'imperativo' in by_id['q65']['choices'][0]['text']
    if unit['order'].startswith('119.'):
        assert '*Prohíben que' in by_id['q39']['prompt']
        assert 'indicativo' in by_id['q60']['prompt']
    if unit['order'].startswith('120.'):
        assert by_id['q29']['correct_answer'] == 'b'
        assert by_id['q42']['choices'][0]['text'].startswith('Me parece bien')
...
assert totals == Counter({'content_changed': 25, 'unchanged': 255}), totals
