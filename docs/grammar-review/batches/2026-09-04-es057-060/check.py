#!/usr/bin/env python3
"""Read-only validation of the ES 057-060 editorial batch against its baseline."""
...
units = [u for u in gr.inventory() if u["language"] == "es" and u["order"].startswith(("057.", "058.", "059.", "060.")) and u["bank"] != "verbs"]
assert len(units) == 28 and sum(u["count"] for u in units) == 511
preexisting = gr.read(BATCH / "preexisting-edits.json")
...
def baseline(path):
    ...
    if path.relative_to(ROOT).as_posix() == preexisting["source"]:
        question = next(q for q in data["questions"] if q["id"] == preexisting["question_id"])
        question.clear()
        question.update(preexisting["baseline_question"])
    return data
...
print("PASS: all 511 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality")
