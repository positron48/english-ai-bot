#!/usr/bin/env python3
"""Read-only validation of the ES 077-080 editorial batch against its baseline."""
...
units = [u for u in gr.inventory() if u["language"] == "es" and u["order"].startswith(("077.", "078.", "079.", "080.")) and u["bank"] != "verbs"]
assert len(units) == 16 and sum(u["count"] for u in units) == 366
...
print("PASS: all 366 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality")
