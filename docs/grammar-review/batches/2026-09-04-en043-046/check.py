...
units = [u for u in gr.inventory() if u["language"] == "en" and u["order"].startswith(("043.", "044.", "045.", "046.")) and u["bank"] != "verbs"]
assert len(units) == 24 and sum(u["count"] for u in units) == 377
BASELINES = {
...
print('PASS: all 377 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
