#!/usr/bin/env python3
"""Evidence checks for the producir/prohibir/prometer/proponer review batch."""
...
BASE = Path("/tmp/grammar-es-verbs-producir-proponer-baseline")
LEMMAS = {
    "producir": "9728ef0f3905ee3e27e40b12",
    "prohibir": "85800971abf4b12a44023e95",
    "prometer": "468f6527f8af9392bd1e97fc",
    "proponer": "2f12206bda90b5ce5cf423ed",
}
SCOPES = [
...
def main():
    ...
    for lemma, report_id in LEMMAS.items():
        ...
        assert new_ids == old_ids, f"{lemma}: scope/person/number order changed"
        ...
        for scope_index, (scope, mood, tense) in enumerate(SCOPES):
            ...
            for slot_index, card in enumerate(scope_cards):
                ...
                subject_tokens = card["question_es_with_blank"].lower().replace(",", "").replace(".", "").split()
                alternatives = [
                    {"yo"}, {"tú"}, {"él", "ella", "usted"},
                    {"nosotros", "nosotras"}, {"vosotros", "vosotras"},
                    {"ellos", "ellas", "ustedes"},
                ][slot_index]
                assert alternatives.intersection(subject_tokens), f"{lemma} {scope}: subject marker missing"
    ...
    checkpoints = [json.loads(line) for line in (
        ROOT / "docs/grammar-review/batches/2026-09-06-es-verbs-producir-proponer/checkpoints.jsonl"
