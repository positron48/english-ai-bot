#!/usr/bin/env python3
"""Read-only validation of the EN 115-118 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-en115118-baseline')
...
units = [
    ...
    if u['language'] == 'en'
    and u['order'].startswith(('115.', '116.', '117.', '118.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 25 and sum(u['count'] for u in units) == 422
...
assert q('perfect_infinitive_gerund_to/03-questions.json', 'q18')['choices'][1]['text'] == 'have left'
assert q('116/en.06.b6_theory_common_errors.questions.json', 'q10')['choices'][1]['text'].startswith('Presented at')
assert q('dangling_participles_common_mistake/03-questions.json', 'q46')['correct_answer'] == 'false'
assert q('117/en.03.b3_theory_fixes.questions.json', 'q7')['correct_answer'] == 'a'
assert 'the team updated the policy' in q('118/en.02.b2_theory_construction_menu.questions.json', 'q10')['choices'][2]['text']
...
assert totals == Counter({'unchanged': 336, 'content_changed': 86}), totals
print('Changes:', dict(totals))
print('PASS: all 422 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
