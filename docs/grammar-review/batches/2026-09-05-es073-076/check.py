#!/usr/bin/env python3
"""Read-only validation of the ES 073-076 editorial batch against its baseline."""
...
units = [u for u in gr.inventory() if u["language"] == "es" and u["order"].startswith(("073.", "074.", "075.", "076.")) and u["bank"] != "verbs"]
assert len(units) == 28 and sum(u["count"] for u in units) == 462
...
print("PASS: all 462 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality")
