#!/usr/bin/env python3
"""Read-only validation of the ES 141-144 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es141144-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('141.', '142.', '143.', '144.'))
    and u['bank'] != 'verbs'
]
...
for i, unit in enumerate(units):
    ...
    if unit['order'].startswith('141.'):
        assert 'qué decisión' in by_id['q141_b3_m3']['prompt']
        assert by_id['q141_b5_f2']['correct_answer'] == 'redactó'
        assert by_id['q141_b6_f2']['correct_answer'] == 'debería'
        assert by_id['q141_b1_e1']['choices'][0]['text'] == 'Se dice que el museo cerrará el lunes.'
        assert 'pasivo con *ser*' not in by_id['q141_b1_match']['pairs'][3]['right']
    if unit['order'].startswith('142.'):
        assert by_id['q142_b2_m4']['choices'][3]['text'].startswith('Он учитывает временную рамку')
        assert by_id['q142_b2_r1']['correct_answer'] == 'Afirmó que el semáforo aún está averiado.'
        assert by_id['q142_b3_m2']['choices'][0]['text'] == 'No sabía cuándo salía el tren.'
        assert by_id['q142_b4_f1']['correct_answer'] == 'por'
        assert 'Вариант с *él* грамматичен' in by_id['q142_b6_m4']['explanation']
    if unit['order'].startswith('143.'):
        assert 'при утверждении факта возможен indicativo' in by_id['q143_b2_match']['pairs'][1]['right']
        assert by_id['q143_b2_r1']['prompt'].startswith('Дополните пересказ вчерашней просьбы')
        assert 'исполнитель понятен из *nos*' in by_id['q143_b5_m3']['choices'][2]['text']
        assert by_id['q143_b6_e1']['choices'][0]['text'] == 'Me sugirió que hable con el responsable del proyecto.'
    if unit['order'].startswith('144.'):
        assert 'грамматически возможно' in by_id['q144_b1_e1']['explanation']
        assert by_id['q144_b2_f2']['correct_answer'] == 'última'
        assert by_id['q144_b3_f2']['correct_answer'] == 'aquí'
        assert by_id['q144_b6_f1']['correct_answer'] == 'mañana'
        assert 'тоже грамматичен' in by_id['q144_b6_m3']['explanation']
        assert '*aun así*' in by_id['q144_b6_match']['pairs'][2]['right']
...
assert totals == Counter({'content_changed': 86, 'unchanged': 202}), totals
