#!/usr/bin/env python3
"""Evidence checks for the evitar/exigir/existir/explicar review batch."""
...
BASE = Path("/tmp/grammar-es-verbs-evitar-explicar-baseline")
LEMMAS = {
    "evitar": "3ff0d53a56cbfcfe33258dcd",
    "exigir": "7702d4da16d3a6505528ae88",
    "existir": "eeda9eb2e86ae464c9282b47",
    "explicar": "dc5d2bd4cc35095117e6e664",
}
SCOPES = [
...
def jehle_forms():
    ...
    # `existir` is absent from the bundled Jehle CSV. These 16 six-slot rows
    # are transcribed from the RAE DLE conjugation table for existir.
    rae = [
        ["existo", "existes", "existe", "existimos", "existís", "existen"],
        ["existía", "existías", "existía", "existíamos", "existíais", "existían"],
        ["existí", "exististe", "existió", "existimos", "exististeis", "existieron"],
        ["existiré", "existirás", "existirá", "existiremos", "existiréis", "existirán"],
        ["existiría", "existirías", "existiría", "existiríamos", "existiríais", "existirían"],
        ["he existido", "has existido", "ha existido", "hemos existido", "habéis existido", "han existido"],
        ["había existido", "habías existido", "había existido", "habíamos existido", "habíais existido", "habían existido"],
        ["hube existido", "hubiste existido", "hubo existido", "hubimos existido", "hubisteis existido", "hubieron existido"],
        ["habré existido", "habrás existido", "habrá existido", "habremos existido", "habréis existido", "habrán existido"],
        ["habría existido", "habrías existido", "habría existido", "habríamos existido", "habríais existido", "habrían existido"],
        ["exista", "existas", "exista", "existamos", "existáis", "existan"],
        ["existiera", "existieras", "existiera", "existiéramos", "existierais", "existieran"],
        ["existiere", "existieres", "existiere", "existiéremos", "existiereis", "existieren"],
        ["haya existido", "hayas existido", "haya existido", "hayamos existido", "hayáis existido", "hayan existido"],
        ["hubiera existido", "hubieras existido", "hubiera existido", "hubiéramos existido", "hubierais existido", "hubieran existido"],
        ["hubiere existido", "hubieres existido", "hubiere existido", "hubiéremos existido", "hubiereis existido", "hubieren existido"],
    ]
    for index, (_, mood, tense) in enumerate(SCOPES):
        found[("existir", mood, tense)] = rae[index]
    return found
...
def main():
    ...
    for lemma, report_id in LEMMAS.items():
        ...
        expected_source = "RAE DLE" if lemma == "existir" else "Jehle"
        assert all(expected_source in q["note"] for q in report["questions"]), (
            f"{lemma}: report cites the wrong form source"
        )
    ...
    checkpoints = [json.loads(line) for line in (
        ROOT / "docs/grammar-review/batches/2026-09-06-es-verbs-evitar-explicar/checkpoints.jsonl"
