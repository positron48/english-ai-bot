#!/usr/bin/env python3
"""Read-only validation of the ES chapters 041–044 editorial batch against its baseline."""
import json
import pathlib
...
units = [u for u in gr.inventory() if u["language"] == "es" and u["order"].startswith(("041.", "042.", "043.", "044.")) and u["bank"] != "verbs"]
assert len(units) == 28 and sum(u["count"] for u in units) == 531
BASELINES = {
    "spanish-grammar": "501044644954d5aaa474f7d1eaee99518a4ebd9d",
    "english-grammar": "1ec076ada14b88fba983a482f3cfcdc6d37ee7ca",
}

def baseline(path):
    main = ROOT / "courses/spanish-grammar/chapters/042.es.grammar.present_irregulars_reflexives_gustar.build_speak_i_usually_do_happening_now/03-questions.json"
    final = main.parent / "05-final.json"
    if path == main:
        return gr.read(ROOT / "docs/grammar-review/batches/2026-09-04-es041-044/baseline/042/03-questions.json")
    if path == final:
        return gr.read(ROOT / "docs/grammar-review/batches/2026-09-04-es041-044/baseline/042/05-final.json")
...
print('PASS: all 531 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
