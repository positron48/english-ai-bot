#!/usr/bin/env python3
"""Read-only validation of the EN 087-090 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-en8790-baseline')
...
units = [
    ...
    if u['language'] == 'en'
    and u['order'].startswith(('087.', '088.', '089.', '090.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 25 and sum(u['count'] for u in units) == 450
...
assert totals == Counter({'unchanged': 311, 'content_changed': 139}), totals
print('Changes:', dict(totals))
print('PASS: all 450 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
