#!/usr/bin/env python3
"""Evidence checks for the calcular/cambiar/caminar verb-form review batch."""
...
BASE = Path("/tmp/grammar-es-verbs-calcular-caminar-baseline")
LEMMAS = {
    "calcular": "a886b4d40772eb2434bab0d4",
    "cambiar": "4b44495701de15a66b0844d2",
    "caminar": "7055b874c85831454af0dda3",
}
SCOPES = [
...
def authoritative_forms():
    ...
    with path.open(encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle):
            if row["infinitive"] in {"calcular", "caminar"}:
    ...
    cambiar = [
        ...
        ["cambiaba", "cambiabas", "cambiaba", "cambiábamos", "cambiabais", "cambiaban"],
        ["cambié", "cambiaste", "cambió", "cambiamos", "cambiasteis", "cambiaron"],
        ["cambiaré", "cambiarás", "cambiará", "cambiaremos", "cambiaréis", "cambiarán"],
        ["cambiaría", "cambiarías", "cambiaría", "cambiaríamos", "cambiaríais", "cambiarían"],
        ["he cambiado", "has cambiado", "ha cambiado", "hemos cambiado", "habéis cambiado", "han cambiado"],
        ["había cambiado", "habías cambiado", "había cambiado", "habíamos cambiado", "habíais cambiado", "habían cambiado"],
        ["hube cambiado", "hubiste cambiado", "hubo cambiado", "hubimos cambiado", "hubisteis cambiado", "hubieron cambiado"],
        ["habré cambiado", "habrás cambiado", "habrá cambiado", "habremos cambiado", "habréis cambiado", "habrán cambiado"],
        ["habría cambiado", "habrías cambiado", "habría cambiado", "habríamos cambiado", "habríais cambiado", "habrían cambiado"],
        ["cambie", "cambies", "cambie", "cambiemos", "cambiéis", "cambien"],
        ["cambiara", "cambiaras", "cambiara", "cambiáramos", "cambiarais", "cambiaran"],
        ["cambiare", "cambiares", "cambiare", "cambiáremos", "cambiareis", "cambiaren"],
        ["haya cambiado", "hayas cambiado", "haya cambiado", "hayamos cambiado", "hayáis cambiado", "hayan cambiado"],
        ["hubiera cambiado", "hubieras cambiado", "hubiera cambiado", "hubiéramos cambiado", "hubierais cambiado", "hubieran cambiado"],
        ["hubiere cambiado", "hubieres cambiado", "hubiere cambiado", "hubiéremos cambiado", "hubiereis cambiado", "hubieren cambiado"],
    ]
    for (_, mood, tense), values in zip(SCOPES, cambiar):
        found[("cambiar", mood, tense)] = values
    return found
...
def main():
    ...
    for lemma, report_id in LEMMAS.items():
        ...
        expected_reference = "RAE" if lemma == "cambiar" else "Jehle"
    ...
    checkpoints = [json.loads(line) for line in (
        ROOT / "docs/grammar-review/batches/2026-09-06-es-verbs-calcular-caminar/checkpoints.jsonl"
