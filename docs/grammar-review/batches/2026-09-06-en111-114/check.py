#!/usr/bin/env python3
"""Read-only validation of the EN 111-114 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-en111114-baseline')
...
units = [
    ...
    if u['language'] == 'en'
    and u['order'].startswith(('111.', '112.', '113.', '114.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 25 and sum(u['count'] for u in units) == 449
...
# Focused regressions for the material defects corrected in this batch.
...
assert q('wish_if_only_would/03-questions.json', 'q15')['prompt'].find('___ read') >= 0
assert q('en.01.b1_theory_wish_present.questions.json', 'q9')['correct_answer'] == 'b'
assert q('en.03.b3_theory_alternatives.questions.json', 'q7')['choices'][0]['text'].startswith('If I had studied')
assert q('gerund_vs_infinitive_basic/03-questions.json', 'q3')['prompt'].find('___ read') >= 0
assert q('verb_patterns_v_ing/03-questions.json', 'q60')['choices'][0]['text'].startswith("I didn't mean")
...
assert totals == Counter({'unchanged': 351, 'content_changed': 98}), totals
print('Changes:', dict(totals))
print('PASS: all 449 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
