#!/usr/bin/env python3
"""Read-only validation of the ES 081-084 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es8184-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('081.', '082.', '083.', '084.'))
    and u['bank'] == 'chapter'
]
...
assert totals == Counter({'unchanged': 265, 'content_changed': 23}), totals
