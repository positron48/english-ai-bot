#!/usr/bin/env python3
"""Read-only validation of the EN chapters 031–034 editorial batch against its baseline."""
import json
import pathlib
...
units = [u for u in gr.inventory() if u["language"] == "en" and u["order"].startswith(("031.", "032.", "033.", "034.")) and u["bank"] != "verbs"]
assert len(units) == 25 and sum(u["count"] for u in units) == 396
BASELINES = {
...
for lang,course in [('en','english-grammar')]:
...
print('PASS: all 396 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
