#!/usr/bin/env python3
"""Read-only validation of the ES chapters 053-056 editorial batch against its baseline."""
import json
import pathlib
...
units = [u for u in gr.inventory() if u["language"] == "es" and u["order"].startswith(("053.", "054.", "055.", "056.")) and u["bank"] != "verbs"]
assert len(units) == 28 and sum(u["count"] for u in units) == 597
BASELINES = {"spanish-grammar": "501044644954d5aaa474f7d1eaee99518a4ebd9d"}

def baseline(path):
    relative = path.relative_to(ROOT / "courses")
    course = relative.parts[0]
    source = pathlib.Path(*relative.parts[1:]).as_posix()
    return json.loads(subprocess.check_output(
        ["git", "-C", str(ROOT / "courses" / course), "show", f"{BASELINES[course]}:{source}"], text=True
    ))
...
for i, u in enumerate(units):
    ...
    old = baseline(p)
    now = gr.read(p)
    r = gr.read(ROOT / u["report"])
    assert r["source"] == u["source"]
    assert r["source_sha256"] == u["source_sha256"], (p, "stale source fingerprint")
    assert r["context_sha256"] == u["context_sha256"], (p, "stale context fingerprint")
    assert {k: v for k, v in old.items() if k != "questions"} == {k: v for k, v in now.items() if k != "questions"}, p
    oldqs = old["questions"]
    qs = now["questions"]
    assert len(qs) == len(oldqs) == u["count"]
    def identity(q):
        return (q["id"], q["type"], q.get("difficulty"), q.get("theory_block_id"), q.get("chapter_id"), q.get("concept_id"), [x["id"] for x in q.get("choices", [])])
    ...
    for before, q, row in zip(oldqs, qs, r["questions"]):
        ...
        if q["type"] == "reorder":
            assert q.get("translation_ru") and 2 <= len(q["correct_answer"].split()) <= 10
    if u["bank"] == "training":
        ss = importlib.util.spec_from_file_location("gen", p.parents[3] / "scripts/generate-training-pack.py")
        gen = importlib.util.module_from_spec(ss)
        ss.loader.exec_module(gen)
        for q in qs:
            errors = gen.validate_question(q, u["chapter"], blocks)
            assert not errors, (p, q["id"], errors)
        assert all(q["signature"] == gen.question_signature(q) for q in qs), p
        assert len({q["signature"] for q in qs}) == len(qs), p
    else:
        final = gr.read(p.parent / "05-final.json")
        assert final["question_bank"]["questions"] == qs, (p, "source-final mismatch")
    ...
    if u["bank"] == "chapter":
        source = p.parent / "05-final.json"
        dest = ROOT / f'internal/grammarbundle/{u["language"]}/chapters/{u["chapter"]}.json'
    else:
        source = p
        dest = ROOT / "internal/grammartrainingpack" / u["language"] / p.relative_to(p.parents[2])
...
ss = importlib.util.spec_from_file_location("gen", ROOT / "courses/spanish-grammar/scripts/generate-training-pack.py")
gen = importlib.util.module_from_spec(ss)
ss.loader.exec_module(gen)
oldsigns = Counter()
newsigns = Counter()
for p in (ROOT / "courses/spanish-grammar/training_pack/chapters").rglob("*.json"):
...
print("PASS: all 597 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality")
