#!/usr/bin/env python3
"""Read-only validation of the EN 099-102 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-en99102-baseline')
...
units = [
    ...
    if u['language'] == 'en'
    and u['order'].startswith(('099.', '100.', '101.', '102.'))
    and u['bank'] != 'verbs'
]
...
assert totals == Counter({'unchanged': 250, 'content_changed': 177}), totals
