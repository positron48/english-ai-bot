# ES verb forms: comprender, comunicar, condenar

Дата редакторской проверки: 2026-09-06. Независимая проверка ожидается.

## Объём

- `comprender`: 96/96 карточек проверено, `fixed` 96.
- `comunicar`: 96/96 карточек проверено, `fixed` 96.
- `condenar`: 96/96 карточек проверено, `fixed` 96.
...
верхнеуровневые поля сохранены. Для `comunicar` исправлено русское управление:
`сообщить об изменении`, `о решении`, `о результате`, `о дате`.
Источники точечно синхронизированы с `internal/grammartrainingpack`;
`make training-pack` не запускался.

## Основные исправления

- Все формы сверены с локальной базой Jehle. Исправлено 33 неверных
  `surface_form`: 6 в `comprender`, 19 в `comunicar` и 8 в `condenar`.
- В `comprender` исправлены все шесть форм futuro perfecto de subjuntivo с
  `hubiere`.
- В `comunicar` исправлены первое лицо presente, весь pretérito anterior,
  futuro de subjuntivo и futuro perfecto de subjuntivo.
- В `condenar` восстановлены `condenemos`, `condenéis` и все шесть форм
  futuro de subjuntivo.
- В исходных файлах обнаружено 62 карточки с повторяющимися вариантами. Теперь
...
- Исправлены шаблонные контексты, падежное управление и переводы без нужного
  временного значения. Редкие времена отмечены как книжные, официальные,
...
- `python3 docs/grammar-review/batches/2026-09-06-es-verbs-comprender-condenar/check.py`
...
- Сохранность 24967 файлов вне разрешённого набора — 0 изменённых, 0 пропавших.
