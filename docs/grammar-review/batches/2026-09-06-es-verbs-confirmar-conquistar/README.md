# ES verb forms: confirmar, conocer, conquistar

Дата редакторской проверки: 2026-09-06. Независимая проверка ожидается.

## Объём

- `confirmar`: 96/96 карточек проверено, `fixed` 96.
- `conocer`: 96/96 карточек проверено, `fixed` 96.
- `conquistar`: 96/96 карточек проверено, `fixed` 96.
...
верхнеуровневые поля сохранены. Для `conocer` системно соблюдено русское
управление `знаком с` / `ознакомиться с`; у `confirmar` согласовано число
`el dato` — `сведение`. Источники точечно синхронизированы с
...
- Все формы сверены с локальной базой Jehle. Исправлено 39 неверных
  `surface_form`: 8 в `confirmar`, 18 в `conocer` и 13 в `conquistar`.
- В `confirmar` исправлены `confirmes`, `confirméis` и весь futuro
  perfecto de subjuntivo.
- В `conocer` восстановлены futuro de subjuntivo, pretérito perfecto de
  subjuntivo и futuro perfecto de subjuntivo.
- В `conquistar` исправлено третье лицо множественного числа imperfecto de
  subjuntivo, futuro de subjuntivo и futuro perfecto de subjuntivo.
- В исходных файлах обнаружено 68 карточек с повторяющимися вариантами. Теперь
  в каждой карточке четыре уникальные формы того же scope, правильная встречается
  ровно один раз.
- Исправлены шаблонные контексты, падежное управление и переводы без нужного
  временного значения. Редкие времена отмечены как книжные, юридические,
  конкурентные или регламентные употребления.
...
- `python3 docs/grammar-review/batches/2026-09-06-es-verbs-confirmar-conquistar/check.py`
...
- Сохранность 24981 файла вне разрешённого набора — 0 изменённых, 0 пропавших.
