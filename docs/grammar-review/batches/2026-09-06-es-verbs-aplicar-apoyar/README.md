# ES verb forms: aplicar, apostar, apoyar

Дата редакторской проверки: 2026-09-06. Независимая проверка ожидается.

## Объём

- `aplicar`: 96/96 карточек проверено, `fixed` 96.
- `apostar`: 96/96 карточек проверено, `fixed` 96.
- `apoyar`: 96/96 карточек проверено, `fixed` 96.
...
- Все формы сверены с локальной базой Jehle. Исправлено 14 неверных
  `surface_form`: 2 в `aplicar`, по 6 в `apostar` и `apoyar`.
- В `aplicar` восстановлены ударения в `apliquéis` и `aplicáremos`.
- В `apostar` исправлены чередование `o` → `ue`, pretérito anterior и формы
  presente/futuro de subjuntivo.
- В `apoyar` весь futuro perfecto de subjuntivo больше не подменяется futuro
...
- В исходных файлах обнаружено 63 карточки с повторяющимися вариантами. Теперь
  ...
  ровно один раз.
- Исправлены оборванные задания, смешение времён и неестественные переводы вроде
  «экологическая причина» и «когда буду иметь время». Редкие времена явно
  отмечены как книжные или юридические употребления.
...
- `python3 docs/grammar-review/batches/2026-09-06-es-verbs-aplicar-apoyar/check.py`
...
- Сохранность 24848 файлов вне разрешённого набора — 0 изменённых, 0 пропавших.
