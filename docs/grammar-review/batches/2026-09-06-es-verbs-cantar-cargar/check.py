#!/usr/bin/env python3
"""Evidence checks for the cantar/caracterizar/cargar verb-form review batch."""
...
BASE = Path("/tmp/grammar-es-verbs-cantar-cargar-baseline")
LEMMAS = {
    "cantar": "c40738a0989c6acd76d8b62b",
    "caracterizar": "533bc40faa0dfdabed5fa62e",
    "cargar": "3762b1b27eb5da5ed8c87291",
}
SCOPES = [
...
def authoritative_forms():
    ...
    with path.open(encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle):
            if row["infinitive"] in {"caracterizar", "cargar"}:
    ...
    for (_, mood, tense), values in zip(SCOPES, cantar):
        found[("cantar", mood, tense)] = values
    return found
...
def main():
    ...
    for lemma, report_id in LEMMAS.items():
        ...
        expected_reference = "RAE" if lemma == "cantar" else "Jehle"
    ...
    checkpoints = [json.loads(line) for line in (
        ROOT / "docs/grammar-review/batches/2026-09-06-es-verbs-cantar-cargar/checkpoints.jsonl"
