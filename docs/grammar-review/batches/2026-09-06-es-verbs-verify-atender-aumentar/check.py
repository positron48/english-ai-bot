#!/usr/bin/env python3
"""Final evidence checks for the atender/atraer/atravesar/aumentar verification batch."""
...
BASE = Path("/tmp/grammar-es-verbs-verify-atender-aumentar-baseline")
...
def main():
    ...
    for lemma, item in manifest.items():
        ...
        assert all(
            question["id"] == f"{card['scope']}:{card['person']}:{card['number']}"
            for card, question in zip(current["cards"], report["questions"])
        ), f"{lemma}: report IDs do not match source coordinates"
