#!/usr/bin/env python3
"""Read-only validation of the ES 101-104 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es101104-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('101.', '102.', '103.', '104.'))
    and u['bank'] != 'verbs'
]
...
assert totals == Counter({'content_changed': 74, 'unchanged': 214}), totals
print('Changes:', dict(totals))
print('PASS: all 288 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
