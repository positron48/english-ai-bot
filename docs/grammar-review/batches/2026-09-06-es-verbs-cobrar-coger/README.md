# ES verb forms: cobrar, cocinar, coger

Дата редакторской проверки: 2026-09-06. Независимая проверка ожидается.

## Объём

- `cobrar`: 96/96 карточек проверено, `fixed` 96.
- `cocinar`: 96/96 карточек проверено, `fixed` 96.
- `coger`: 96/96 карточек проверено, `fixed` 96.
...
верхнеуровневые поля сохранены. Для `coger` выбрано нейтральное транспортное
значение. Источники точечно синхронизированы с
`internal/grammartrainingpack`; `make training-pack` не запускался.

## Основные исправления

- Все формы сверены с локальной базой Jehle. Исправлено 22 неверных
  `surface_form`: 10 в `cobrar`, 6 в `cocinar` и 6 в `coger`.
- В `cobrar` исправлены `cobrabais`, `cobrasteis`, `cobráremos`, `cobrareis`
  и все шесть форм futuro perfecto de subjuntivo с `hubiere`.
- В `cocinar` восстановлены все шесть форм futuro de subjuntivo:
  `cocinare`–`cocinaren`.
- В `coger` восстановлены все шесть форм futuro de subjuntivo:
  `cogiere`–`cogieren`.
- В исходных файлах обнаружено 52 карточки с повторяющимися вариантами. Теперь
  ...
  ровно один раз.
- Исправлены шаблонные и неестественные контексты и переводы. Редкие времена
  явно отмечены как книжные либо юридические/регламентные употребления.
...
- `python3 docs/grammar-review/batches/2026-09-06-es-verbs-cobrar-coger/check.py`
...
- Сохранность 24939 файлов вне разрешённого набора — 0 изменённых, 0 пропавших.
