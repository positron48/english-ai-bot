#!/usr/bin/env python3
"""Read-only validation of the ES 097-100 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es97100-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('097.', '098.', '099.', '100.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 10 and sum(u['count'] for u in units) == 330
...
assert totals == Counter({'content_changed': 150, 'unchanged': 180}), totals
print('Changes:', dict(totals))
print('PASS: all 330 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
