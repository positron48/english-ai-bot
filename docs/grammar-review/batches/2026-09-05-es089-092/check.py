#!/usr/bin/env python3
"""Read-only validation of the ES 089-092 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es8992-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('089.', '090.', '091.', '092.'))
    and u['bank'] == 'chapter'
]
...
assert totals == Counter({'unchanged': 217, 'content_changed': 71}), totals
