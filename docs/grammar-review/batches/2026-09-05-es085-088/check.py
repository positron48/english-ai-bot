#!/usr/bin/env python3
"""Read-only validation of the ES 085-088 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es8588-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('085.', '086.', '087.', '088.'))
    and u['bank'] == 'chapter'
]
...
assert totals == Counter({'unchanged': 236, 'content_changed': 52}), totals
