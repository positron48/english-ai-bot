#!/usr/bin/env python3
"""Read-only validation of the EN 091-094 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-en9194-baseline')
...
units = [
    ...
    if u['language'] == 'en'
    and u['order'].startswith(('091.', '092.', '093.', '094.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 25 and sum(u['count'] for u in units) == 438
...
assert totals == Counter({'unchanged': 293, 'content_changed': 145}), totals
print('Changes:', dict(totals))
print('PASS: all 438 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
