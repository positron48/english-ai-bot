#!/usr/bin/env python3
"""Read-only validation of the ES 113-116 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es113116-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('113.', '114.', '115.', '116.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 4 and sum(u['count'] for u in units) == 276
...
for i, unit in enumerate(units):
    ...
    if unit['order'].startswith('113.'):
        by_id = {question['id']: question for question in questions}
        assert by_id['q025']['correct_answer'] == 'a', 'ES 113 q025 key regression'
    if unit['order'].startswith('115.'):
        by_id = {question['id']: question for question in questions}
        assert by_id['q52']['correct_answer'] == 'true', 'ES 115 q52 key regression'
    if unit['order'].startswith('116.'):
        by_id = {question['id']: question for question in questions}
        assert '___ trabajado' in by_id['q6']['prompt'], 'ES 116 q6 incomplete sentence regression'
        assert '___ ido' in by_id['q32']['prompt'], 'ES 116 q32 incomplete sentence regression'
...
assert totals == Counter({'content_changed': 38, 'unchanged': 238}), totals
print('Changes:', dict(totals))
print('PASS: all 276 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
