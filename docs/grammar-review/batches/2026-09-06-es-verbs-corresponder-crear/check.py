#!/usr/bin/env python3
"""Evidence checks for the corresponder/cortar/costar/crear verb-form review batch."""
...
BASE = Path("/tmp/grammar-es-verbs-corresponder-crear-baseline")
LEMMAS = {
    "corresponder": "fb900eee762e5dec12bd538d",
    "cortar": "2725646fc1d638b6ce1aae58",
    "costar": "b5843326b9a929ab785ed5ef",
    "crear": "b0a8ca2376d7fe581dffc634",
}
SCOPES = [
...
def authoritative_forms():
    found = {}
    path = ROOT / "resources/verbs/jehle_verb_database.csv"
    with path.open(encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle):
            if row["infinitive"] in {"cortar", "costar", "crear"}:
                found[(row["infinitive"], row["mood"], row["tense"])] = [
                    row[column].strip() for column in FORM_COLS
                ]
    corresponder = [
        ["correspondo", "correspondes", "corresponde", "correspondemos", "correspondéis", "corresponden"],
        ["correspondía", "correspondías", "correspondía", "correspondíamos", "correspondíais", "correspondían"],
        ["correspondí", "correspondiste", "correspondió", "correspondimos", "correspondisteis", "correspondieron"],
        ["corresponderé", "corresponderás", "corresponderá", "corresponderemos", "corresponderéis", "corresponderán"],
        ["correspondería", "corresponderías", "correspondería", "corresponderíamos", "corresponderíais", "corresponderían"],
        ["he correspondido", "has correspondido", "ha correspondido", "hemos correspondido", "habéis correspondido", "han correspondido"],
        ["había correspondido", "habías correspondido", "había correspondido", "habíamos correspondido", "habíais correspondido", "habían correspondido"],
        ["hube correspondido", "hubiste correspondido", "hubo correspondido", "hubimos correspondido", "hubisteis correspondido", "hubieron correspondido"],
        ["habré correspondido", "habrás correspondido", "habrá correspondido", "habremos correspondido", "habréis correspondido", "habrán correspondido"],
        ["habría correspondido", "habrías correspondido", "habría correspondido", "habríamos correspondido", "habríais correspondido", "habrían correspondido"],
        ["corresponda", "correspondas", "corresponda", "correspondamos", "correspondáis", "correspondan"],
        ["correspondiera", "correspondieras", "correspondiera", "correspondiéramos", "correspondierais", "correspondieran"],
        ["correspondiere", "correspondieres", "correspondiere", "correspondiéremos", "correspondiereis", "correspondieren"],
        ["haya correspondido", "hayas correspondido", "haya correspondido", "hayamos correspondido", "hayáis correspondido", "hayan correspondido"],
        ["hubiera correspondido", "hubieras correspondido", "hubiera correspondido", "hubiéramos correspondido", "hubierais correspondido", "hubieran correspondido"],
        ["hubiere correspondido", "hubieres correspondido", "hubiere correspondido", "hubiéremos correspondido", "hubiereis correspondido", "hubieren correspondido"],
    ]
    for (_, mood, tense), values in zip(SCOPES, corresponder):
        found[("corresponder", mood, tense)] = values
    return found
...
def main():
    ...
    for lemma, report_id in LEMMAS.items():
        ...
        # Person/number slots and their order are the stable identity here. The
        # baselines contain noncanonical tense labels, and costar assigns the last
        # 24 slots to preceding scopes; this batch deliberately repairs them.
        ...
        for scope_index, (scope, mood, tense) in enumerate(SCOPES):
            ...
            assert [c["surface_form"] for c in scope_cards] == expected_forms, (
                f"{lemma} {scope}: differs from authoritative paradigm"
            )
        ...
        expected_reference = "RAE" if lemma == "corresponder" else "Jehle"
        assert all(expected_reference in q["note"] for q in report["questions"]), (
            f"{lemma}: review note lacks {expected_reference} reference"
        )

        print(f"{lemma}: PASS 96 cards, exact authoritative forms, contracts and report fingerprint")

    checkpoints = [json.loads(line) for line in (
        ROOT / "docs/grammar-review/batches/2026-09-06-es-verbs-corresponder-crear/checkpoints.jsonl"
