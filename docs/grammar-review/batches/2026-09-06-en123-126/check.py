#!/usr/bin/env python3
"""Read-only validation of the EN 123-126 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-en123126-baseline')
...
units = [
    ...
    if u['language'] == 'en'
    and u['order'].startswith(('123.', '124.', '125.', '126.'))
    and u['bank'] != 'verbs'
]
...
assert 'актуальн' in q('build_write_retell_a/03-questions.json', 'q7')['prompt'].lower()
assert q('123/en.01.b1_theory_retell_structure.questions.json', 'q8')['correct_answer'] == 'b'
assert 'speaker + reporting verb' in q('123/en.01.b1_theory_retell_structure.questions.json', 'q1')['choices'][0]['text']
assert 'обычный backshift' in q('123/en.02.b2_theory_mix_statements_questions.questions.json', 'q6')['prompt']
assert q('124/en.03.b3_theory_contrast_correction.questions.json', 'q1')['choices'][0]['text'] == 'I do agree with your theory.'
assert 'обязательный вспомогательный' in q('emphasis_do_emphasis/03-questions.json', 'q27')['explanation']
assert q('125/en.02.b2_theory_not_only.questions.json', 'q1')['choices'][2]['text'].startswith('Not only did')
assert 'Past Perfect' in q('inversion_never_have_i/03-questions.json', 'q30')['prompt']
assert q('cleft_sentences_it_s/03-questions.json', 'q37')['correct_answer'] == 'b'
assert q('cleft_sentences_it_s/03-questions.json', 'q41')['correct_answer'] == 'She was the person we hired.'
assert q('126/en.04.b4_theory_reversed_cleft.questions.json', 'q2')['choices'][1]['text'] == 'You are the person I trust.'
assert 'также возможно' in q('126/en.05.b5_theory_style_limits.questions.json', 'q1')['explanation']
...
assert totals == Counter({'unchanged': 287, 'content_changed': 146}), totals
