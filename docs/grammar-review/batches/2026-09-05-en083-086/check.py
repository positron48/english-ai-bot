#!/usr/bin/env python3
"""Read-only validation of the EN 083-086 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-en8386-baseline')
...
units = [
    ...
    if u['language'] == 'en'
    and u['order'].startswith(('083.', '084.', '085.', '086.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 25 and sum(u['count'] for u in units) == 447
...
assert totals == Counter({'unchanged': 307, 'content_changed': 140}), totals
