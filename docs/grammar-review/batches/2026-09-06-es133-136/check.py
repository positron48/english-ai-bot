#!/usr/bin/env python3
"""Read-only validation of the ES 133-136 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es133136-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('133.', '134.', '135.', '136.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 4 and sum(u['count'] for u in units) == 252
...
for i, unit in enumerate(units):
    ...
    if unit['order'].startswith('133.'):
        assert by_id['q133_b1_cloze']['correct_answer'] == 'aceptaras|tuviéramos'
        assert by_id['q133_b6_cloze']['correct_answer'] == 'iría|enviara'
        assert by_id['q133_b4_fill']['prompt'].startswith('Вставьте: *El médico me dijo')
        assert by_id['q133_b2_transform']['correct_answer'] == 'Si comiera más verduras, me sentiría mejor.'
        assert by_id['q133_b2_prod']['correct_answer'] == 'Si pudiera, viajaría más.'
    if unit['order'].startswith('134.'):
        assert 'больше не актуален' in by_id['q134_b1_mcq_e']['prompt']
        assert by_id['q134_b2_cloze']['correct_answer'].endswith('enviáramos')
        assert by_id['q134_b3_cloze']['correct_answer'] == 'había|hubiéramos'
        assert by_id['q134_b4_cloze']['correct_answer'] == 'firmaría|entregáramos'
        assert by_id['q134_b5_transform']['correct_answer'] == 'Preguntó dónde estaba el archivo.'
        assert 'presente может сохраниться' in by_id['q134_b6_explain']['explanation']
    if unit['order'].startswith('135.'):
        assert by_id['q135_b1_mcq_m']['choices'][0]['text'] == '*Si tuviera dinero, compraría una casa.*'
        assert by_id['q135_b3_cloze']['prompt'].find('llegarás') > 0
        assert 'гл. 137' in by_id['q135_b4_mcq_h']['choices'][0]['text']
        assert by_id['q135_b6_mcq_h']['choices'][0]['text'].startswith('Это допустимое смешанное условное')
        assert 'habría comprado' in by_id['q135_b6_err']['explanation']
        assert 'гл. 136' not in json.dumps(by_id, ensure_ascii=False)
    if unit['order'].startswith('136.'):
        assert by_id['q136_b2_fill']['correct_answer'] == 'hayáis'
        assert by_id['q136_b2_match']['pairs'][1] == {'left': 'tú', 'right': 'hayas'}
        assert by_id['q136_b4_mcq_m']['choices'][2]['text'] == '*Te llamaré cuando revise el informe.*'
        assert by_id['q136_b4_explain']['prompt'].find('cuando termine') > 0
        assert by_id['q136_b6_match']['pairs'][3]['left'].startswith('*Te llamaré')
...
assert totals == Counter({'content_changed': 102, 'unchanged': 150}), totals
print('Changes:', dict(totals))
print('PASS: all 252 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
