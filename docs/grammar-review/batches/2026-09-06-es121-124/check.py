#!/usr/bin/env python3
"""Read-only validation of the ES 121-124 editorial batch against its baseline."""
...
BASE_DIR = pathlib.Path('/tmp/grammar-es121124-baseline')
...
units = [
    ...
    if u['language'] == 'es'
    and u['order'].startswith(('121.', '122.', '123.', '124.'))
    and u['bank'] != 'verbs'
]
assert len(units) == 4 and sum(u['count'] for u in units) == 280
...
for i, unit in enumerate(units):
    ...
    if unit['order'].startswith('121.'):
        assert '*Creo que vendrá*' in by_id['q22']['explanation']
        assert 'Indicativo también' not in by_id['q40']['explanation']
        assert 'Indicativo' in by_id['q40']['explanation']
        assert by_id['q65']['choices'][1]['text'] == 'vendrá'
    if unit['order'].startswith('122.'):
        assert 'está listo' in by_id['q3']['choices'][1]['text']
        assert by_id['q44']['choices'][1]['text'].startswith('haya ')
        assert 'imperfecto de subjuntivo' in by_id['q44']['choices'][3]['feedback']
        assert 'firmó' in by_id['q49']['choices'][0]['text']
    if unit['order'].startswith('123.'):
        assert 'совпадающие' in by_id['q1']['explanation']
        assert 'imperativo' in by_id['q45']['choices'][0]['text']
        assert 'другое значение' in by_id['q47']['choices'][1]['text']
        assert by_id['q69'] == baseline(path)['questions'][68]
    if unit['order'].startswith('124.'):
        assert 'отдельным субъектом' in by_id['q14']['prompt']
        assert 'может быть реальным' in by_id['q22']['explanation']
        assert '*creo que ir* неверно' in by_id['q47']['explanation']
...
assert totals == Counter({'content_changed': 44, 'unchanged': 236}), totals
print('Changes:', dict(totals))
print('PASS: all 280 IDs/contracts, choices, theory bindings, report fingerprints, signatures and source-final equality')
