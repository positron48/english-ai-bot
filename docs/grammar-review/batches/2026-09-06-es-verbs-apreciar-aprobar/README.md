# ES verb forms: apreciar, aprender, aprobar

Дата редакторской проверки: 2026-09-06. Независимая проверка ожидается.

## Объём

- `apreciar`: 96/96 карточек проверено, `fixed` 96.
- `aprender`: 96/96 карточек проверено, `fixed` 96.
- `aprobar`: 96/96 карточек проверено, `fixed` 96.
...
- Все формы сверены с локальной базой Jehle. Исправлено 55 неверных
  `surface_form`: по 12 в `apreciar` и `aprender`, 31 в `aprobar`.
- У `apreciar` и `aprender` восстановлены оба блока futuro de subjuntivo.
- В `aprobar` исправлены pretérito anterior, полные составные формы с
  `aprobado`, futuro simple/perfecto de subjuntivo и pretérito perfecto de
  subjuntivo.
- Исправлено 18 некорректных значений mood/tense у `aprobar`.
- В исходных файлах обнаружено 74 карточки с повторяющимися вариантами. Теперь
  в каждой карточке четыре уникальные формы того же scope, правильная встречается
  ровно один раз.
- Исправлены оборванные вопросы, подмена `aprender` значением «закончить курс»
  и неестественные русские переводы. Редкие времена явно отмечены как книжные
...
- `python3 docs/grammar-review/batches/2026-09-06-es-verbs-apreciar-aprobar/check.py`
...
- Сохранность 24855 файлов вне разрешённого набора — 0 изменённых, 0 пропавших.
