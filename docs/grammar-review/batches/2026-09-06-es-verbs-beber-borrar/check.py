#!/usr/bin/env python3
"""Evidence checks for the beber/bloquear/borrar verb-form review batch."""
...
BASE = Path("/tmp/grammar-es-verbs-beber-borrar-baseline")
LEMMAS = {
    "beber": "f19ab5dd749b0893af46188b",
    "bloquear": "084a50998e53adc5d29879e0",
    "borrar": "b7d26e551808514cd92d295b",
}
SCOPES = [
...
def authoritative_forms():
    ...
    with path.open(encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle):
            if row["infinitive"] in {"beber", "borrar"}:
                found[(row["infinitive"], row["mood"], row["tense"])] = [
                    row[column].strip() for column in FORM_COLS
                ]
    bloquear = [
        ["bloqueo", "bloqueas", "bloquea", "bloqueamos", "bloqueáis", "bloquean"],
        ["bloqueaba", "bloqueabas", "bloqueaba", "bloqueábamos", "bloqueabais", "bloqueaban"],
        ["bloqueé", "bloqueaste", "bloqueó", "bloqueamos", "bloqueasteis", "bloquearon"],
        ["bloquearé", "bloquearás", "bloqueará", "bloquearemos", "bloquearéis", "bloquearán"],
        ["bloquearía", "bloquearías", "bloquearía", "bloquearíamos", "bloquearíais", "bloquearían"],
        ["he bloqueado", "has bloqueado", "ha bloqueado", "hemos bloqueado", "habéis bloqueado", "han bloqueado"],
        ["había bloqueado", "habías bloqueado", "había bloqueado", "habíamos bloqueado", "habíais bloqueado", "habían bloqueado"],
        ["hube bloqueado", "hubiste bloqueado", "hubo bloqueado", "hubimos bloqueado", "hubisteis bloqueado", "hubieron bloqueado"],
        ["habré bloqueado", "habrás bloqueado", "habrá bloqueado", "habremos bloqueado", "habréis bloqueado", "habrán bloqueado"],
        ["habría bloqueado", "habrías bloqueado", "habría bloqueado", "habríamos bloqueado", "habríais bloqueado", "habrían bloqueado"],
        ["bloquee", "bloquees", "bloquee", "bloqueemos", "bloqueéis", "bloqueen"],
        ["bloqueara", "bloquearas", "bloqueara", "bloqueáramos", "bloquearais", "bloquearan"],
        ["bloqueare", "bloqueares", "bloqueare", "bloqueáremos", "bloqueareis", "bloquearen"],
        ["haya bloqueado", "hayas bloqueado", "haya bloqueado", "hayamos bloqueado", "hayáis bloqueado", "hayan bloqueado"],
        ["hubiera bloqueado", "hubieras bloqueado", "hubiera bloqueado", "hubiéramos bloqueado", "hubierais bloqueado", "hubieran bloqueado"],
        ["hubiere bloqueado", "hubieres bloqueado", "hubiere bloqueado", "hubiéremos bloqueado", "hubiereis bloqueado", "hubieren bloqueado"],
    ]
    for (_, mood, tense), values in zip(SCOPES, bloquear):
        found[("bloquear", mood, tense)] = values
    return found
...
def main():
    ...
    for lemma, report_id in LEMMAS.items():
        ...
        # malformed beber baseline assigned the last 24 slots to the preceding
        # scopes; this batch deliberately repairs those scope coordinates.
        ...
        expected_reference = "RAE" if lemma == "bloquear" else "Jehle"
    ...
    checkpoints = [json.loads(line) for line in (
        ROOT / "docs/grammar-review/batches/2026-09-06-es-verbs-beber-borrar/checkpoints.jsonl"
    ...
    print(f"PASS: {total} cards; 18 checkpoints; no duplicate prompt+answer signatures")
